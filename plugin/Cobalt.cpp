  ...
  if (request.Verb == Web::Request::HTTP_POST) {
    ...
    if ((index.Next() == true) && (index.Next() == true)
      ...
      if (stateControl != nullptr) {
        ...
        }else if ((index.Remainder() == _T("CAPTIONS"))
                   && (request.HasBody() == true)
                   && (request.Body<const Data>()->URL.Value().empty()
                       == false)) {
            _cobalt->SetENV(_T("CAPTIONS"),request.Body<const Data>()->URL.Value());
        }else if ((index.Remainder() == _T("CAPTIONS_SUPPORT"))
                   && (request.HasBody() == true)
                   && (request.Body<const Data>()->URL.Value().empty()
                       == false)) {
            _cobalt->SetENV(_T("CAPTIONS_SUPPORT"),request.Body<const Data>()->URL.Value());
