...
#elif SB_IS(64_BIT)
    hwcap_value |= HasItemInList(flags_buffer, "aes") ? HWCAP_AES : 0;
    hwcap_value |= HasItemInList(flags_buffer, "pmull") ? HWCAP_PMULL : 0;
    hwcap_value |= HasItemInList(flags_buffer, "sha1") ? HWCAP_SHA1 : 0;
    hwcap_value |= HasItemInList(flags_buffer, "sha2") ? HWCAP_SHA2 : 0;
    hwcap_value |= HasItemInList(flags_buffer, "crc32") ? HWCAP_CRC32 : 0;
#endif
  return hwcap_value;
}
