...
bool GetCacheDirectory(char* out_path, int path_size) {
  ...
  SB_LOG(INFO) << "SbSysGetPath:  CacheDirectory = " << std::string(out_path) ;
...
bool GetStorageDirectory(char* out_path, int path_size) {
  ...
  std::string temppath = std::string(home_path.data());

  size_t n = temppath.find_last_not_of(" /\r\n\t");
  // #1 remote the end '/'
  if (n != std::string::npos)
    temppath.erase(n+1, temppath.size() -n);
  // #2 goto the previous directory /xxxx
  n =  temppath.find_last_of('/');
  if (n != std::string::npos)
    temppath.erase(n, temppath.size() -n);
...
bool GetExecutablePath(char* out_path, int path_size) {
  ...
  SB_LOG(INFO) << "SbSysGetPath: ExecutablePath = " << std::string(out_path);
  return true;
}
...
bool GetEvergreenContentPathOverride(char* out_path, int path_size) {
  ...
  SB_LOG(INFO) << "SbSysGetPath: EvergreenContentPathOverride = " << std::string(out_path);
  return true;
}
#endif
...
bool GetExecutableDirectory(char* out_path, int path_size) {
  ...
  *last_slash = '\0';
  SB_LOG(INFO) << "SbSysGetPath: GetExecutableDirectory = " << std::string(out_path);
  return true;
}
...
bool GetTemporaryDirectory(char* out_path, int path_size) {
  ...
  SB_LOG(INFO) << "SbSysGetPath: GetTemporaryDirectory = " << std::string(out_path);
  return true;
}

// Gets the path to the content directory.
bool GetContentDirectory(char* out_path, int path_size) {
  starboard::strlcpy(out_path, "/usr/share/content/data", path_size);
  return true;
  ...
  SB_LOG(INFO) << "SbSysGetPath: GetContentDirectory = " << std::string(out_path);
  return true;
}
}  // namespace
