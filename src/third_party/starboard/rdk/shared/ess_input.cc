...
SbKey KeyCodeToSbKey(uint16_t code) {
  switch (code) {
    ...
    case KEY_F3:
    case KEY_F4:
    //case KEY_F5: // which causes  Page Visibility API for Cobalt 22+ test case failed
                   // when power off TV, box cec send out a 116 F5 key to app.
                   // Cobalt reload the page cause Cobalt process hangup.
                   // On this page press any key, it cause Cobalt process crash.
                   // It should be test page's issue
    case KEY_F6:
    case KEY_F7:
    case KEY_F8:
    case KEY_F9:
    ...
    case KEY_F11:
    case KEY_F12:
      return static_cast<SbKey>(kSbKeyF1 + (code - KEY_F1));
...
void EssInput::OnKeyboardKey(unsigned int key, SbInputEventType type) {
  ...
  bool repeatable =
    ((key_modifiers_ == kSbKeyModifiersNone) && (key != KEY_UNKNOWN)) ||
