...
#else
  audiosink_ = gst_element_factory_make("amlhalasink", NULL);
  g_object_set(G_OBJECT(audiosink_), "direct-mode", FALSE, NULL);
  g_signal_connect(
      audiosink_, "child-added",
      G_CALLBACK(&GStreamerAudioSink::AutoAudioSinkChildAddedCallback), this);
#endif
